...
import supportlib.struct.IntSet.*;
import supportlib.struct.ObjectMap.*;
import supportlib.struct.Queue;
import supportlib.struct.OrderedMap.*;
import supportlib.util.*;
import supportlib.util.io.*;
import supportlib.util.serialization.JsonValue.*;
import supportlib.util.serialization.JsonWriter.*;
...
public class Json{
    ...
    public void writeValue(Object value, Class knownType, Class elementType){
        ...
        try{
            ...
            if(value instanceof supportlib.struct.Queue){
                if(knownType != null && actualType != knownType && actualType != supportlib.struct.Queue.class)
                    throw new SerializationException("Serialization of a Queue other than the known type is not supported.\n"
                    + "Known type: " + knownType + "\nActual type: " + actualType);
                writeArrayStart();
                supportlib.struct.Queue queue = (supportlib.struct.Queue)value;
    ...
    public <T> T readValue(Class<T> type, Class elementType, JsonValue jsonData, Class keytype){
        ...
        if(jsonData.isArray()){
            ...
            if(supportlib.struct.Queue.class.isAssignableFrom(type)){
                supportlib.struct.Queue result = type == supportlib.struct.Queue.class ? new supportlib.struct.Queue() : (Queue)newInstance(type);
                for(JsonValue child = jsonData.child; child != null; child = child.next)
